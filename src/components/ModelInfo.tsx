...
const ModelInfo: React.FC<{ trained?: boolean; members?: number }> = ({
  trained = true,
  members = 1,
}) => {
  return (
    <section className="rounded-2xl border bg-gradient-to-br from-card to-card/50 shadow-sm p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <Badge
          ...
          className={`text-sm px-3 py-1 ${
            trained ? "bg-green-500 hover:bg-green-600" : ""
          }`}
        >
          {trained ? (
            <>
      ...
      <div className="space-y-4">
        <div className="p-4 rounded-xl bg-muted/30">
          <h4 className="font-semibold text-sm mb-2 text-[hsl(var(--brand))]">
            OpenCV LBPH Face Recognizer
          </h4>
          <p className="text-xs text-muted-foreground">
            Local Binary Pattern Histogram with enhanced Haar cascade detection
          </p>
        </div>
